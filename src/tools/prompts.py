"""MCP Prompts for video editing guidance and context helpers."""
import logging
from typing import Dict, Any

from ..server_deps import _operation_history, _performance_stats
...
def register(mcp, deps):
    ...
    # We need references to list_files and get_file_info tool functions.
    # Since they are registered in file_management module, we call the underlying logic directly.
    from ..models import FileInfo

    async def _list_files_internal():
        """Internal helper to get files list without going through MCP."""
        files = []
        video_files = []
        audio_files = []
        image_files = []
        try:
            ...
            if not source_dir.exists():
                return {"files": [], "error": "Source dir not found"}
        ...
        return {"files": files, "video_files": video_files, "audio_files": audio_files, "image_files": image_files}

    async def _get_file_info_internal(file_id: str):
        """Internal helper for file info."""
        ...
        return {"basic_info": basic_info, "media_info": media_info}
...
- Compatibility considerations"""
        ...
        file_info = await _get_file_info_internal(file_id)
...
Remember: Start small, test thoroughly, and always keep your source files safe during batch operations!"""
    ...
    # CONTEXT SYSTEM helper functions (not registered as tools, used internally)

    async def _get_files_summary() -> str:
        """Get a summary of available files - CONTEXT HELPER"""
        try:
            files_result = await _list_files_internal()
